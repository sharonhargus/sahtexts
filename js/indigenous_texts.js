/*
	...
	Here BD/Belle Deacon files correspond to files with a single long audio, and Chapman files
	refer to files with a folder of short audio clips.
*/
...
/* This should hae the format "//depts.washington.edu/<ACCOUNT NAME>/annsaver/index.cgi", where account name is the text account for the website.  For example, the kwadacha website looks like
	"//depts.washington.edu/sahtexts/annsaver/index.cgi"
	Make sure to include the quotes. 
*/
		this.f.action=window.location.protocol+"\\depts.washington.edu/sahtexts/annsaver/index.cgi";
		this.f.method="POST";
...
function ChapmanPage(){
	...
	/* The first <CHANGE-ME> should be a pointer to the media account for the website. For example,
	the kwadacha path is "//depts.washington.edu/sahtext2/"
	Make sure to include quotes and the final slash.
	The second <CHANGE-ME> should have the form "/<FILE TYPE>/0.<FILE TYPE>", where file type is the
	kind of audio files.  For example, the kwadacha path is"/wav/0.wav"
	Make sure to include quotes and the initial slash.
	*/
	src.setAttribute('src', "//depts.washington.edu/sahtext2/"+mediapath+"/wav/0.wav");
...
*/
										window.location.hostname+"/sahtexts/timestamps/"+
